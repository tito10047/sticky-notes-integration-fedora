...
export function safe_disconnect(handler_id, instance) {
    // NOTE: despite the parameter names, all call sites pass (GObject, numeric_handler_id).
    // 'handler_id' is the GObject and 'instance' is the numeric ID returned by .connect().
    // The body GObject.disconnect(numeric_id) is therefore correct.
    // Original body had operands swapped (numeric_id.disconnect(GObject)), causing a silent
    // TypeError on every call — signals were never actually disconnected.
    // Parameter names intentionally left unchanged to avoid a noisy rename across call sites.
    if (instance) {
        try {
            handler_id.disconnect(instance);
        } catch (error) {
            return false;
        }
    }
